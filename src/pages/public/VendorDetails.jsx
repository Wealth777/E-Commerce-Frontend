...
const VendorDetails = () => {
  ...
  const getVendorChatUserId = () => {
    const possibleIds = [
      vendorInfo?.user,
      vendorInfo?.userId,
      vendorInfo?.authUser,
      vendorInfo?.account,
      vendorInfo?.accountId,
      vendorInfo?.owner,
      vendorInfo?.ownerId,
      vendorInfo?.createdBy,
      vendorInfo?.createdById,
      vendorInfo?.vendorUser,
      vendorInfo?.vendorUserId,
      vendorInfo?.vendor?.user,
      vendorInfo?.vendor?.userId,
      vendorInfo?._id,
      vendorInfo?.id,
      vendorId,
    ];
  ...
  const handleStartChat = async () => {
    if (!isAuthenticated) {
      showToast('Please login as a buyer to start a chat.', 'error');
      navigate('/login', { state: { from: `/vendor/${vendorId}` } });
      return;
    }

    if (role !== 'buyer') {
      showToast('Only buyers can start a chat with vendors from this page.', 'error');
      return;
    }

    const targetUserId = getVendorChatUserId();

    if (!targetUserId) {
      showToast('Unable to identify the vendor account for chat. The vendor details API must return the vendor user ID as user, userId, owner, or accountId.', 'error');
      return;
    }
    ...
    try {
      ...
      if (!conversationId) {
        showToast('Chat was created, but the conversation ID was not returned.', 'error');
        navigate(
      ...
      showToast(
        getMessage(error, 'Failed to start chat with vendor'),
        'error'
      );
    } finally {
              ...
              >
                ...
                <div className="text-left">
                  ...
                  <p className="text-sm font-semibold">
                    {startChatLoading ? 'Opening chat...' : 'Message vendor'}
                  </p>
                </div>
              </button>
            </div>
