  ...
  return (
    <div className={`min-h-screen ${bgColor} py-12`}>
      <div className="max-w-7xl mx-auto px-4">
        <h1 className={`text-3xl font-bold ${textColor} mb-8`}>My Orders</h1>
              ...
              <div key={order.id} className={`${cardBg} shadow rounded-lg p-6`}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-red-600">₦{order.total?.toLocaleString()}</p>
                  </div>
                  <div>
                    <p className={`text-sm ${secondaryText}`}>Items</p>
                    <p className={textColor}>{order.items?.length || 0}</p>
                  </div>
                  <button className="text-red-600 hover:underline mt-4">View Details</button>
                </div>
              </div>
            ))}
          </div>
