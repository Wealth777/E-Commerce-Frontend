...
const RatingForm = ({ initialData = {}, onSubmit, onCancel, loading = false, showComment = true, submitText = 'Save rating' }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!rating) return;
    const data = { rating, ratingValue: rating };
    if (showComment && comment.trim()) data.comment = comment.trim();
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 rounded-2xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 p-5 shadow-sm">
      <div>
        <label className="mb-2 block text-sm font-bold text-gray-700 dark:text-gray-200">Rating</label>
        <RatingStars value={rating} onChange={setRating} size="text-2xl" />
      </div>
      {showComment && (
        <div>
          <label className="mb-2 block text-sm font-bold text-gray-700 dark:text-gray-200">Comment</label>
          <textarea value={comment} onChange={(e) => setComment(e.target.value)} rows="3" maxLength="500" className="w-full rounded-xl border border-gray-200 bg-white p-3 text-sm text-gray-900 outline-none focus:ring-2 focus:ring-green-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white" placeholder="Share a short comment" />
        </div>
      )}
      <div>
      ...
      <div className="flex flex-wrap gap-3">
        <button disabled={loading || !rating} className="rounded-xl bg-green-600 px-5 py-3 text-sm font-bold text-white hover:bg-green-700 disabled:opacity-60">
